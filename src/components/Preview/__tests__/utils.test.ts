...
function parseItems(items: { type: string, text: string, depth: number }[]): string {
	let tex = ''
	let previousDepth = 0
	let beginCount = 0
	items.forEach((item, index) => {
		 ...
		 */
		if (index === 0) { // the first list item.
			tex += '\\begin{itemize}'
			previousDepth = depth
			beginCount += 1
		}
		...
		tex += `\\item ${text}`
		if (index === items.length - 1) { // current list is exhausted.
